...
async def get_or_create_admin() -> dict:
    ...
    user = await get_user_by_email(_ADMIN_EMAIL)
    if user is None:
...
async def create_tenant(name: str, owner_id: str) -> dict:
    ...
    from cognee.modules.users.tenants.methods import create_tenant as cognee_create_tenant

    tenant_id = await cognee_create_tenant(
...
async def add_user_to_tenant(
    ...
    from cognee.modules.users.tenants.methods import add_user_to_tenant as cognee_add_user

    try:
        await cognee_add_user(
...
async def render_graph_visualization_html(
    ...
    cognee_user = None
    if user_id:
        cognee_user = await get_user(UUID(user_id))
